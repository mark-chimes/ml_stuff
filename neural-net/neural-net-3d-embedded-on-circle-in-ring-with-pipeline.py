...
plt.suptitle('Training and Test Datasets Before Scaling')
standardFlatLimitsAndLabels(plt)
...
for a, b in angles: 
    # Plot the points with true surface.
    ax = plt.axes(projection='3d')
    plt.suptitle('Training data before scaling')
    standardAxLimitsAndLabels(ax)
    ax.view_init(a, b)
    # floatAndProjections(sx, sy, sz, 0.3, 0.03, a, b)
    # predictionWireframeFloatAndPredictions(generatingFunction, 0.05, 0.01, a, b)
    floatAndProjections(X_train[:,0:1], X_train[:,1:2], X_train[:,2:3], main_alph=0.5, proj_alph=0.01)
    plt.show()
...
y_train_colors = np.array(['blue' if x > 0.5 else 'red' for x in y_train]).T
y_test_colors = np.array(['cyan' if x > 0.5 else 'pink' for x in y_test]).T
plt.suptitle('Training Dataset After Scaling')
standardFlatLimitsAndLabels(plt)
plt.scatter(X_train[:,0:1], X_train[:,1:2], marker='2', c=y_train_colors)
plt.scatter(X_test[:,0:1], X_test[:,1:2], marker=',', c=y_test_colors)
plt.show()
...
for a, b in angles: 
    # Plot the points with true surface.
    ax = plt.axes(projection='3d')
    plt.suptitle('Training data after scaling')
    standardAxLimitsAndLabels(ax)
    ax.view_init(a, b)
    # floatAndProjections(sx, sy, sz, 0.3, 0.03, a, b)
    # predictionWireframeFloatAndPredictions(generatingFunction, 0.05, 0.01, a, b)
...
for layers in [1,2,3,5,10]:
    neuron_specs = np.ndarray.tolist(np.linspace(1,15-layers,3).astype(int))
    for neurons in neuron_specs:
        hidden_layers = tuple(neurons for i in range(layers))
        print("Training neural net with layers: " + str(hidden_layers))
        
        clf = MLPClassifier(solver='lbfgs', max_iter=10000, alpha=1e-5, hidden_layer_sizes=hidden_layers, random_state=1)
        clf.fit(X_train, y_train)
    
        y_predict_train = clf.predict(X_train)
        y_predict_test = clf.predict(X_test)
        y_predict_colors_train = np.array(['lightgreen' if x > 0.5 else 'orange' for x in y_predict_train]).T
        y_predict_colors_test = np.array(['blue' if x > 0.5 else 'red' for x in y_predict_test]).T

        gx = gy = np.arange(-1, 1, 0.01)
        gX, gY = np.meshgrid(gx, gy)
        gZ = np.column_stack((gX.ravel(), gY.ravel(), generatingFunction(gX, gY).ravel()))
        y_predict_for_mesh = clf.predict(gZ)
        
        title = 'Neural Net Classification with layers:' + str(hidden_layers)
        plt.suptitle(title)
        standardFlatLimitsAndLabels(plt)
        Z_predict_colors = np.array(['cyan' if x > 0.5 else 'pink' for x in y_predict_for_mesh]).T
        
        plt.scatter(gX, gY, marker='.', c=Z_predict_colors)
        plt.scatter(X_test[:,0], X_test[:,1], marker='o', c=y_test_colors)
        plt.scatter(X_test[:,0], X_test[:,1], marker='3', c=y_predict_colors_test)
        #plt.scatter(sbx, sby, marker='.', color='red')
        plt.show()
