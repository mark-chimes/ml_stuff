    ...
    for activation in activations:
        for layers in np.ndarray.tolist(np.linspace(min_layers,max_layers,num_layer_types).astype(int)):
            ...
            for neurons in neuron_specs:
                ...
                print('Training neural net with ' + solver + ' / ' + activation +  ' and layers: ' + str(hidden_layers))
                
                clf = MLPClassifier(solver='lbfgs', max_iter=max_iter, alpha=1e-5, hidden_layer_sizes=hidden_layers, random_state=1)
                ...
                standardFlatLimitsAndLabels(plt)
                title = 'Neural Net Classification with ' + solver + ' / ' + activation + ' and layers ' + str(hidden_layers)
                plt.suptitle(title)
