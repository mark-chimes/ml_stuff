...
XY = np.column_stack((all_points[:,:-1], np.ones(X.shape))) # first two columns and a ones to embed in 3D
...
standardFlatLimitsAndLabels(plt)
plt.scatter(XY[:,0:1], XY[:,1:2], marker='.', c=Z_colors)
plt.show()
...
standardFlatLimitsAndLabels(plt)
plt.scatter(XY_train[:,0:1], XY_train[:,1:2], marker='2', c=Z_train_colors)
plt.scatter(XY_test[:,0:1], XY_test[:,1:2], marker=',', c=Z_test_colors)
...
for iterations in [1,2,5,10, 20]:
    ...
    Z_mesh = clf.predict(np.c_[xx.ravel(), yy.ravel(), np.ones(xx.ravel().shape)])
    ...
    plt.scatter(XY_train[:,0:1], XY_train[:,1:2], marker='.', c=Z_train_colors)
